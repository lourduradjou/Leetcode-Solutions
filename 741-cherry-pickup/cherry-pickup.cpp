...
public:
    ...
    int findPaths(int row1, int col1, int row2, int col2, vector<vector<int>> &grid) {
        ...
        int cnt1 = findPaths(row1, col1 + 1, row2, col2 + 1, grid); // h h
        int cnt2 = findPaths(row1 + 1, col1, row2, col2 + 1, grid); // v h
        int cnt3 = findPaths(row1, col1 + 1, row2 + 1, col2, grid); // h v
        int cnt4 = findPaths(row1 + 1, col1, row2 + 1, col2, grid); // v v

        cherries += max({cnt1, cnt2, cnt3, cnt4});
        return dp[row1][col1][row2][col2] = cherries;
    }

    int cherryPickup(vector<vector<int>>& grid) {
        memset(dp, 0, sizeof(dp));
        int ans = findPaths(0, 0, 0, 0, grid);
        return (ans < 0) ? 0: ans;
    }
};